...
Všechny konfigurační konstanty jsou v `src/biomcp/constants.py`:
...
services:
  czechmedmcp:
    ...
    volumes:
      - biomcp-cache:/root/.cache/biomcp
volumes:
  biomcp-cache:
```
...
- **HTTP/SSE** — pro vzdálený přístup (`czechmedmcp run --transport sse`)
