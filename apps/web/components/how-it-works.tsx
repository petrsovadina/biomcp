...
export function HowItWorks() {
  return (
    <section id="jak-to-funguje" className="bg-gray-50 dark:bg-[#030303] py-24 md:py-32">
          ...
          {steps.map((s, i) => (
            <FadeIn key={s.num} delay={i * 0.15} className="flex gap-6">
              <div className="flex flex-col items-center">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-blue-100 dark:bg-blue-500/20 text-sm font-bold text-blue-600 dark:text-blue-400">
                  {s.num}
                </div>
                {i < steps.length - 1 && (
                  <div className="mt-2 h-full w-px bg-gray-200 dark:bg-white/[0.08]" />
                )}
              </div>
              <div className="pb-8">
