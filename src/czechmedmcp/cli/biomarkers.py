...
def search_biomarkers_cli(
    ...
    annotations, use 'czechmedmcp variant search' with MyVariant.info.

    Examples:
        # Search by biomarker name
        czechmedmcp biomarker search "PD-L1"

        # Search by type
        czechmedmcp biomarker search --type reference_gene

        # Search for specific biomarker
        czechmedmcp biomarker search "EGFR mutation"
    """
    try:
        results = asyncio.run(
            search_biomarkers(
