...
def search_interventions_cli(
    ...
    Examples:
        # Search by drug name
        czechmedmcp intervention search pembrolizumab

        # Search by type
        czechmedmcp intervention search --type Drug

        # Search for devices
        czechmedmcp intervention search "CAR T" --type Biological

        # Search without synonyms
        czechmedmcp intervention search imatinib --no-synonyms
    """
    try:
        results = asyncio.run(
...
def get_intervention_cli(
    ...
    Example:
        czechmedmcp intervention get INT123456
    """
    try:
