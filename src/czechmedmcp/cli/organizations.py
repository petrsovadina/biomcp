...
def search_organizations_cli(
    ...
    Examples:
        # Search by name
        czechmedmcp organization search "MD Anderson"

        # Search by type
        czechmedmcp organization search --type academic

        # Search by location
        czechmedmcp organization search --city Boston --state MA

        # Combine filters
        czechmedmcp organization search Cancer --type industry --state CA
    """
    try:
        results = asyncio.run(
...
def get_organization_cli(
    ...
    Example:
        czechmedmcp organization get ORG123456
    """
    try:
